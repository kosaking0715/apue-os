...
{
	struct iovec iov[2];
	char asciiptrlen[PTRLEN_BUFSZ];
	int len;

	if ((db->ptrval = ptrval) < 0 || ptrval > PTR_MAX)
		err_quit("_db_writeidx: invalid ptr: %lld", (long long)ptrval);

	snprintf(db->idxbuf, sizeof(db->idxbuf),
			 "%s%c%lld%c%ld\n",
			 key, SEP, (long long)db->datoff, SEP, (long)db->datlen);

	len = (int)strlen(db->idxbuf);
	if (len < IDXLEN_MIN || len > IDXLEN_MAX)
		err_dump("_db_writeidx: invalid length");

	snprintf(asciiptrlen, sizeof(asciiptrlen),
			 "%*lld%*d", PTR_SZ, (long long)ptrval, IDXLEN_SZ, len);
	...
	if (writev(db->idxfd, &iov[0], 2) != (ssize_t)(PTR_SZ + IDXLEN_SZ + len))
